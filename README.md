...
To make the setup easier, you can setup the vscode [devcontainer](.devcontainer). Launch the `LangMesh devcontainer`.  Refer to [Prerequisites](https://gitlab.aws.dev/langmesh/resources/-/blob/main/docs/Prerequisites.md?ref_type=heads) to setup and launch the devcontainer. Once the container launches, hit the "+" (New Terminal) icon to start a `bash` terminal. Launch the `bash` terminal in [deployment](./deployment) folder. All the steps below can be done localy as well if you have Node and terraform installed.
