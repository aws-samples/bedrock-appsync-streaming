...
"""lambda_handler.py:
    ...
    SNS records and uses LangMesh SDK to interact with FMs in Amazon SageMaker
    ...
    This function uses LangMesh SDK Amazon SageMaker and Amazon Bedrock streaming
    endpoints to interact with FMs.  When the streaming endpoint returns partial
    response (partial tokens) from FM, the function calls AWS AppSync GraphQL mutation.
    The mutation trigger asynchronous delivery of the FM partial response to
    be sent over to the client that is subscribed to the mutation; the client
    must use AWS AppSync subscription for this purpose.  This completes the
    asynchronous delivery of FM response to the calling application over web sockets.
"""
...
def lambda_handler(event, context):
    ...
    It uses LangMesh SDK streaming endpoint to interact with the FM.
